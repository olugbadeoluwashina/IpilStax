...
import {fakeProjectRepo, createTestProjectFactory } from './fake_project_repo.ts';
...
Deno.test("CREATEPROJECTUC: test project can be created successfully", async () => {
  ...
  const project = await projectUsecase.execute({ name: "Test Project", description: "A project for testing"});
...
Deno.test("CREATEPROJECTUC: should return the available project code", async () => {
  const repo = fakeProjectRepo();
  repo.create(createTestProjectFactory({ id: "project-id", name: "Test Project", projectCode: "TESPROJ"}));
  const projectUsecase = new CreateProjectUseCase(repo);
